...
RSpec.describe "チャットルームの削除機能", type: :system do
  ...
  it 'チャットルームを削除すると、関連するメッセージがすべて削除されていること' do
    ...
    # メッセージ情報を5つDBに追加する
    FactoryBot.create_list(:message, 5, room_id: @room_user.room.id, user_id: @room_user.user.id)   # messageはroomとuserにもassociateしているので、
          # posts = []
          # 5.times do |i|
          #   posts << Faker::Lorem.sentence
          #   fill_in "message_content", with: posts[i]
          #   click_on("送信")
          # end
